...
- **systemd 裸机 / 一键安装**：`install.sh` / `install.ps1` 先在该 profile 的
  node_modules 里 `npm install` dsh-files 的三个运行时依赖，再把
  `custom-plugins/*`（含 vendored `dsh-files`）拷进
  `$DSH_HOME/profiles/assistant/node_modules/`（loader 与 client-modules 都从
  profile 目录解析插件包）。**顺序不能反**：npm 会裁剪多余包，先拷贝后安装
  会把插件删掉。
...
- **备份**：`assistant-data` volume（会话）+（若需保留上传文档）挂 volume 的 `FMS_WORKSPACE_DIR` + Rails 正常备份（含 mcp_query_logs）。上传文档默认 TTL 7 天自动清扫。
